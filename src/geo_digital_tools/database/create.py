...
class ColumnBuilder:
    ...
    def validate_columns(self):
        ...
        if len(column_names)!=len(set(column_names)):
            gdte.KnownException(f'Column Builder - Unsupported Type : Column - {column_name} Type {column_var}')
...
def dict_raise_on_duplicates(ordered_pairs):
    ...
    for k, v in ordered_pairs:
        if k in d:
           gdte.KnownException(f'GDT - Duplicate Keys detected in Databse Config : {k}')
        else:
           d[k] = v
    return d
