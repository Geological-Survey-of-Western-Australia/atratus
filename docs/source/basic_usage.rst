...
.. Seealso::
    More detail can be found in the api docs :py:class:`gswa-atratus.cygnet.Step`.
...
Another challenge about geodata harmonisation is the distinguishing between problems that can be solved by a developer and problems that can be solved by a geoscientist. **GDT** handles this by raising two types of exceptions:
 ...
 - :py:class:`gswa-atratus.KnownException` - This Exception is intended to be raised when the cause of the issue/error is known. Ideally this is something that can be explained to a geoscientist to either fix or at least be aware of.
 - :py:class:`gswa-atratus.CodeError` - This Exception is intended to be raised by any uncaught exceptions, or any issue that a developer needs to consider and discuss. Often it will be used for discovering new edge cases.
